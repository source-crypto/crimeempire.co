...
import { MapPin, TrendingUp, Link as LinkIcon, Swords } from 'lucide-react';
...
export default function Territories() {
  ...
  const initiateBattleMutation = useMutation({
    mutationFn: async (territory) => {
      if (!playerData.crew_id) {
        throw new Error('You must be in a crew to attack territories');
      }
      ...
      await base44.entities.CrewActivity.create({
        ...
        description: `Attacking ${territory.name}`,
        player_id: playerData.id,
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={battleDialogOpen} onOpenChange={setBattleDialogOpen}>
        <DialogContent className="glass-panel border-purple-500/20 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-white">Battle Arena</DialogTitle>
          </DialogHeader>
          {currentBattle && playerData && (
            <BattleArena
              ...
              <TabsContent value="attack" className="space-y-4">
                <Card className="glass-panel border-red-500/20">
                  <CardHeader className="border-b border-red-500/20">
                    <CardTitle className="text-white">Enemy Territories</CardTitle>
                  </CardHeader>
                  <CardContent className="p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {allTerritories
                        .filter(t => t.controlling_crew_id !== playerData.crew_id)
                        .map((territory) => (
                          <div
                            key={territory.id}
                            className="p-4 rounded-lg bg-slate-900/30 border border-red-500/10"
                          >
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="font-semibold text-white">{territory.name}</h4>
                              {territory.is_contested ? (
                                <Badge className="bg-orange-600">Contested</Badge>
                              ) : (
                                <Badge className="bg-gray-600">Available</Badge>
                              )}
                            </div>
                            <div className="text-sm text-gray-400 mb-3">
                              Defense: {territory.defense_bonus || 0}
                            </div>
                            <Button
                              size="sm"
                              className="w-full bg-gradient-to-r from-red-600 to-orange-600"
                              onClick={() => initiateBattleMutation.mutate(territory)}
                              disabled={territory.is_contested || initiateBattleMutation.isPending}
                            >
